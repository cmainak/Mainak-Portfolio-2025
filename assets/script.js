/* ============================================================
   ...
   /* ============================================================
   5.2 Hero Texture Parallax (The "Live" Effect)
   ============================================================ */
(function () {
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;

  gsap.to(".hero-graphic-layer img", {
    yPercent: 15, // Moves the image slightly as you scroll down
    ease: "none",
    scrollTrigger: {
      trigger: "#hero",
      start: "top top",
      end: "bottom top",
      scrub: true // Locks the movement to your scroll speed
    }
  });
})();

document.addEventListener("DOMContentLoaded", () => {
  // 1. Force visibility immediately for About page content
  // so we don't rely on scroll triggers for static text
  const aboutElements = document.querySelectorAll('.editorial-about .will-animate');
  aboutElements.forEach(el => {
    el.style.opacity = '1';
    el.style.transform = 'translateY(0)';
  });

  // 2. Only initialize ScrollTriggers if GSAP is loaded
  if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
    // Your existing GSAP triggers go here...
  }
});
...
(function() {
  ...
  if (!progressBar) return; // Exit if the bar isn't on this page
  ...
  window.addEventListener('scroll', function() {
    ...
    // Calculate percentage and apply it to the width
